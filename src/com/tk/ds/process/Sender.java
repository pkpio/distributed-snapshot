...
public class Sender extends Process implements Runnable {
	...
	public void run() {
		...
		while (true) {
			...
			if (processes.getQueue().hasElements()){
				MessageMoney message = processes.getQueue().remove();
	 ...
	 * @param message
	 */
	public void sendToProcess(MessageMoney message) {
	 ...
	 * @param msg
	 */
	public void sendToGUI(MessageGUI message) {
	 ...
	 * @param receiver
	 */
	void sendObject(Object object, int receiver){
		try{
